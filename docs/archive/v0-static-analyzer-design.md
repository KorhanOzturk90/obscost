# v0 static-analyzer design (archived)

**Status: frozen.** This is the original v0 spec's design for the
static-analysis (`check`) side of promcost — the `Meter` live tier, the
`scan`/`explain`/`rewrite`/`pint-config` subcommands, the `PC-L0x`/`PC-F0x`
check tiers and the rewrite engine. None of it beyond the static tier
(`PC-S01`–`PC-S06`) was ever built.

It was moved out of `AGENTS.md` on 2026-09-18, after
[ADR 0004](../adr/0004-finops-pivot-scope-and-sequencing.md) re-scoped the
product around tenant cost allocation. Keep it for reference: the parts
about PromQL cost structure come back for pull-request forecasting and for
explaining *why* an expensive rule is expensive. Do not treat it as current
design or as a to-do list — `docs/adr/` and `PRODUCT-DIRECTION.md` are what
current work follows.

The full original spec this was summarised from is
[`promcost-v0-spec.md`](promcost-v0-spec.md).
