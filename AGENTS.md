...
`promcost cost` (`internal/cli/cost.go`) is ADR 0004 build steps 1 and 3, tenant showback, for ADR 0003's pools 1 (ingester memory), 6 (query path) and 7 (ruler CPU); `--pool` picks a subset. It splits each pool across tenants by its driver and renders "`analytics` uses 62% of ingester memory — the equivalent of 5.0 of your 8 ingesters", in currency only if promcost.yaml's `inventory:` block prices the pool. It is a separate command from `report` because the two share almost no inputs. The arithmetic lives in `internal/cost` (pure, no I/O: exhaustive shares, "not measured"/"not costed" never rendered as 0, an assumption trail on every figure) and the PromQL in `internal/cost/mimirdrivers`, which follows ADR 0003's "[A] How each driver must be read" (pool 1: sum-then-average subquery, the ingest-storage guard, replication divisor applied in `internal/cost` so it shows in the trail; pools 6 and 7: per-tenant counters, `increase()` over the window, no divisor). Pool 6's driver is a chain — fetched chunk bytes, samples processed, series fetched, and only then query seconds, which the measurement flags as a fallback because time is contention-sensitive. Pool 7 is rule-evaluation seconds; `cortex_ruler_query_seconds_total` only decides whether rules are evaluated locally or remotely (its query time is *nested inside* the evaluation time, so it is never subtracted), and under remote evaluation the query work is pool 6's. A multi-component pool (the query path) is priced with `inventory.pools.<id>.components`, a partial list prices what it lists as a lower bound, and `AllocateAll` totals across pools only inside `cost.CrossPool`, next to the coverage it leaves out (ADR 0003 [A]): with nothing priced there is no total, and the "fraction of platform cost" exists only if the operator sets `inventory.platform_cost_month`. Both it and `mimirmetrics` query Mimir through the shared `internal/promapi` client. It is a snapshot over one window; comparing windows waits for ADR 0005 (#31).
