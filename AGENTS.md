...
`promcost cost --rule-outputs` (issue #37, `internal/ruleoutput`) adds pool 1's sub-tenant drill-down that names a rule: each recording rule's `record:` name is the metric it writes, so its series are counted in the owning tenant's TSDB (queried *as that tenant*, not as the metrics tenant) with batched `count by (__name__) ({__name__=~"a|b|..."})` instant queries, chunked and concurrency-limited. Rule definitions come from `--dir` or the ruler API for `--tenant`, exactly as in `report`. Four outcomes are kept apart: series, no series right now (a measured zero), not measured (nil, with the reason), and alerting rules, which write no series and are never shown as zero-cost. Shares divide by pool 1's already-deduplicated per-tenant figure, so the replication factor is whatever `internal/cost` resolved; with none known the share is withheld. The count is point-in-time. Don't position this as the main ingestion driver: ADR 0003 [A] measured rule output at ~3.6% of ingestion on a mixin-shaped corpus.
