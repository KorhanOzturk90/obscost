...
The product direction has since expanded beyond the original static-analyzer spec — see `PRODUCT-DIRECTION.md` (the current thesis: observed workload attribution, not just PromQL linting) and `docs/runtime-telemetry-notes.md`. `promcost report` (`internal/cli/report.go`) is the first slice of that: it joins observed `RuleExecution` telemetry (`internal/rule/execution.go`'s `RuleID`/`RuleExecution`, ingested via `internal/telemetry` — currently only the portable `internal/telemetry/ndjson` source; a real Mimir-log-format adapter is a deliberately deferred follow-up, see that package's doc comment) against loaded rule definitions in `internal/attribution`, producing ranked tenant/rule workload shares. This is fully independent of `check`/`internal/analyzer` — neither reads the other's output yet (that join is a later milestone per `PRODUCT-DIRECTION.md`).
